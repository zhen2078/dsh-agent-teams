...
import type { ActivityTeam, ActivityTask } from './ActivityPanel.tsx'
...
function fillOf(task: ActivityTask, team: ActivityTeam): number {
...
export function progressRows(team: ActivityTeam, limit = 5): readonly LotteryRow[] {
 ...
 */
export function wheelSegments(team: ActivityTeam): readonly WheelSegment[] {
  const members: WheelSegment[] = team.members.slice(0, WHEEL_SIZE).map((member) => ({
    label: member.name,
    kind: 'member',
    art: memberArtUrl(member.name, member.role),
  }))
  const segments: WheelSegment[] = [...members]
